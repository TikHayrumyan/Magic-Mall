...
const SecBlog = () => {
  ...
  const { id, content, thumbnail, title, title_am, title_ru, date, description, description_am, description_ru } =
    blogPageData.loading && blogPageData.data[0];
        ...
        {blogPageData.loading && (
          <Link href={`/singleBlogPage/${id}`} key={id}>
            <div className={styles.summerTrend}>
              <div>
                <Image className={styles.img} alt="img" src={HOST_API_IMG + thumbnail} width={749} height={481} />
              </div>
              <div className={styles.aboutTrends}>
                <div className={styles.title}>
                  {lang == "en" ? shorten(title, 80) : lang == "ru" ? shorten(title_ru, 65) : shorten(title_am, 66)}
                </div>
                <div className={styles.dateBLock}>
                  <div>
                    <img alt="calendarIcon" src="/img/blogPage/secBlog/calendarIcon.svg" />
                  </div>
                ...
                <div className={styles.descrip}>
                  {lang == "en"
                    ...
                    : lang == "ru"
                    ? shorten(description_ru, 140)
                    : shorten(description_am, 137)}
                </div>
                <div className={styles.btnBlock}>
                  <button className={styles.button}>{translate.buttonRead[lang]}</button>
                  <div className={styles.arrow}>
                    <img alt="arrow" src="/img/blogPage/secBlog/arrow.svg" />
                  </div>
                </div>
                <div className={styles.pattern}>
                  <img alt="pattern" className="pattern" src="/img/blogPage/secBlog/pattern.svg" />
                </div>
              </div>
            </div>
          </Link>
